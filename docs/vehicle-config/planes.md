...
### Gravity-era tuning guidance

New-flight-model planes are balanced around real downward gravity. Treat `NewFlightGravity` as the physical baseline the wing and engine must overcome, not as a replacement for lift or thrust tuning. Start from the global `NewFlightGravity` value for ordinary fighters and attack aircraft, then adjust documented aerodynamic values first: `StallSpeed`, `StallRecoverySpeed`, `StallLiftLoss`, drag terms, throttle response, low-throttle lift retention, climb energy loss, and control authority.

Use per-aircraft `NewFlightGravity`/`FlightGravity`/`GravityOverride` only when the vehicle role justifies a different weight feel: heavy bombers and transports can use slightly higher gravity, very light drones or special experimental aircraft can use small deviations, and VTOL/special aircraft should still be checked against their non-fixed-wing behavior. Avoid copying a weak gravity override across the roster; low-energy or stalled aircraft should naturally lose altitude.

Because there is no separate documented wing-lift coefficient or `TakeoffSpeed` key, takeoff and climb behavior should be validated through the documented stall, recovery, throttle, drag, ceiling, and climb-energy formulas. `StallRecoverySpeed` should remain above `StallSpeed`, `MaxSafeSpeed` should remain above `CompressibilitySpeed`, and combat-flap overspeed should remain low enough that flaps help landing/low-speed turns without becoming a permanent high-speed buff.
